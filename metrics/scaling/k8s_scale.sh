...
run() {
	...
	for reqs in $(seq ${STEP} ${STEP} ${NUM_PODS}); do
		...
		# FIXME unless we want to start with multiple replicas, we may want to
		# just leave this to the input templates
		sed -e "s|@REPLICAS@|1|g" \
			-e $runtime_command \
			-e "s|@DEPLOYMENT@|${deployment}-${reqs}|g" \
			-e "s|@LABEL@|${LABEL}|g" \
		...
		kubectl rollout status --timeout=${wait_time}s deployment/${deployment}-${reqs}
...
cleanup() {
	...
	local start_time=$(date +%s%N)
	for dep in $(kubectl get deployments --no-headers | awk '$1 ~ /^busybox-/ { print $1 }'); do
		kubectl delete deployment --wait=true --timeout=${delete_wait_time}s $dep || true
	done
